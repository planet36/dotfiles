# SPDX-FileCopyrightText: Alexei Boronine
...
sRGB_to_XYZ_matrix = [[0.41239079926595945, 0.35758433938387796, 0.18048078840183432],
 ...
 [0.019330818715591835, 0.11919477979462596, 0.9505321522496607]]

# XYZ-to-sRGB matrix
XYZ_to_sRGB_matrix = [[3.2409699419045217, -1.5373831775700935, -0.4986107602930034],
 [-0.9692436362808801, 1.8759675015077206, 0.04155505740717568],
 [0.05563007969699364, -0.20397695888897652, 1.0569715142428784]]
...
def _get_bounds(l):
    ...
    while _g < 3:
        c = _g
        _g += 1
        m1 = XYZ_to_sRGB_matrix[c][0]
        m2 = XYZ_to_sRGB_matrix[c][1]
        m3 = XYZ_to_sRGB_matrix[c][2]
        _g1 = 0
        while _g1 < 2:
            t = _g1
            _g1 += 1
...
def xyz_to_rgb(_hx_tuple):
    return (
        _from_linear(_dot_product(XYZ_to_sRGB_matrix[0], _hx_tuple)),
        _from_linear(_dot_product(XYZ_to_sRGB_matrix[1], _hx_tuple)),
        _from_linear(_dot_product(XYZ_to_sRGB_matrix[2], _hx_tuple)))
...
def rgb_to_xyz(_hx_tuple):
    ...
    return (_dot_product(sRGB_to_XYZ_matrix[0], rgbl),
            _dot_product(sRGB_to_XYZ_matrix[1], rgbl),
            _dot_product(sRGB_to_XYZ_matrix[2], rgbl))
...
def rgb_to_hex(_hx_tuple):
    ...
    return '#{:02x}{:02x}{:02x}'.format(
            ...
            int(math.floor(_hx_tuple[2] * 255 + 0.5)))
