...
std::vector<double> get_load_avg(int num_samples = max_samples)
{
	...
	std::vector<double> load_avg(num_samples);
	...
	if (num_samples_ret == -1)
		...
		load_avg.resize(num_samples_ret);

	return load_avg;
}

std::vector<unsigned int> get_load_avg_int(int num_samples = max_samples)
{
	...
	{
		load_avg_int.push_back(x * factor);
	}
